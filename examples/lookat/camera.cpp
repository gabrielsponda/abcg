...
/* void Camera::togle(float speed) {
  // Compute forward vector (view direction)
  auto const forward{glm::normalize(m_at - m_eye)};
  // Compute vector to the left
  auto const left{glm::cross(m_up, forward)};
  // Compute vector upwards and backwards
  auto const up{glm::normalize(glm::cross(forward, left))};
  auto const back{-forward};

  // Move eye and center backwards
  m_eye += back * speed;
  m_at += back * (0.25f * speed);

  // Move eye and center diagonally upwards
  m_eye += up * speed;
  m_at += up * (0.25f * speed);
...
/* void Camera::togle(float speed) {
  ...
  // Rotate camera around its local forward vector
  glm::mat4 transform{1.0f};
  transform = glm::translate(transform, m_eye);
  transform = glm::rotate(transform, speed, forward);
  transform = glm::translate(transform, -m_eye);
